  ...
  <UCard>
    ...
    <div class="mt-3 border border-primary-100/80 bg-primary-50/30 rounded-sm p-8 flex gap-8 items-center">
      <div class="flex flex-col items-center">
        ...
        <BaseRating :model-value="detail.rating" disabled size="lg" class="mt-2" />
      </div>
    ...
    <div class="flex flex-col mt-1 divide-y">
      <div v-for="review in data?.data" :key="`review-${review.user_name}`" class="flex gap-3 py-4">
        <UAvatar :src="review.user_photo" :alt="review.user_name" icon="i-heroicons:user" size="lg" img-class="object-cover" />
        <div class="flex-1">
          ...
          <div v-if="review.attachments?.length" class="flex gap-2 flex-wrap mt-2">
            <NuxtImg v-for="attach in review.attachments" :key="attach" width="72px" height="72px" fit="cover" format="webp" :src="attach" />
          </div>
        </div>
      </div>
