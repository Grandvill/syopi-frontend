    ...
    >
      ...
      <form class="grid grid-cols-2 gap-5 mt-5" @submit.prevent="handleSave">
        ...
        <UFormGroup :error="v$.receiver_phone.$errors?.[0]?.$message">
          <UInput v-model="form.receiver_phone" placeholder="Nomor Telepon" size="lg" />
        </UFormGroup>
        <UFormGroup class="col-span-2" :error="v$.city.uuid.$errors?.[0]?.$message || v$.province.uuid.$errors?.[0]?.$message">
          <FeatureProfileAddressInputSelectCity v-model:city="form.city" v-model:province="form.province" />
        </UFormGroup>
        ...
        <UFormGroup :error="v$.detail_address.$errors?.[0]?.$message" class="col-span-2">
          <UTextarea v-model="form.detail_address" placeholder="Nama Jalan, Gedung, No. Rumah" size="lg" />
        </UFormGroup>
        <UFormGroup :error="v$.address_note.$errors?.[0]?.$message" class="col-span-2">
          <UInput v-model="form.address_note" placeholder="Detail Lainnya (Cth: Blok / Unit No., Patokan)" size="lg" />
        </UFormGroup>
        <div class="col-span-2">
          <label class="text-sm font-normal text-black/80"> Tandai Sebagai: </label>
        ...
        <div class="flex justify-end mt-6 gap-2 col-span-2">
          ...
          <UButton type="submit" label="Ok" class="max-w-36" block :loading="status === 'pending' || statusEdit === 'pending'" />
        </div>
      </form>
    </UCard>
  </UModal>
...
const { execute: edit, status: statusEdit, error: errorEdit } = useSubmit(computed(() => `/server/api/address/${form.value?.uuid}`));
