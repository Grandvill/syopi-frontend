    ...
    <form @submit.prevent="handleSubmit">
      <UFormGroup
        ...
        :ui="{
          ...
          label: {
            wrapper: 'pt-2',
            base: 'text-normal text-black/55 text-base w-28',
          },
        }"
        :error="!!v$.phone.$errors?.[0]?.$message"
      >
        <UInput v-model="form.phone" size="lg" />
        <p v-if="v$.$invalid" class="text-sm text-red-500 mt-1">{{ v$.phone.$errors?.[0]?.$message }}</p>
        <UButton type="submit" label="Selanjutnya" class="mt-8" :loading="status === 'pending'" :disabled="v$.$invalid" />
      </UFormGroup>
    </form>
  </div>
</template>
...
const { execute, status, error } = useSubmit('/server/api/profile', { method: 'PATCH' });
