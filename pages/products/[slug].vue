  ...
  <UContainer v-else as="section" class="flex flex-col gap-5 py-5">
      ...
      <div class="product-seller">
        <div class="flex gap-6 items-center w-96">
          ...
          <div>
            ...
            <UButton color="white" size="xs" class="mt-2" :to="`/shop/${detailProduct.seller.username}`"> <IconShop /> Kunjungi Toko </UButton>
          </div>
        </div>
    ...
    <div v-if="detailProduct.other_product?.length" class="flex flex-col gap-4 mt-2">
      <div class="flex justify-between gap-2 items-cente">
        ...
        <UButton variant="link" class="font-thin" :padded="false" :to="`/shop/${detailProduct.seller.username}`"> Lihat Semua <UIcon name="i-heroicons:chevron-right" /> </UButton>
      </div>
...
const titleMeta = computed(() => `Beli ${detailProduct.value?.name} Hanya Rp${detailProduct.value?.price_sale ? salePrice.value : rawPrice.value}`);
