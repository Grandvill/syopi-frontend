            ...
            <div class="mt-4 space-y-6">
                ...
                <SellerFormGroup label="Video Produk">
                    <div class="flex gap-6 items-center">
                        <FeatureSellerProductMedia :max="1" type="video" icon="i-heroicons:video-camera-solid"/>
                        <div class="text-xs text-black/50">
                            <p>File video maks. harus 30Mb dengan resolusi tidak melebihi 1280 x 1280px.</p>
                            <p>Durasi: 10-60 detik</p>
                            <p>Format: MP4</p>
                            <p>Catatan: Kamu dapat menampilkan produk saat video sedang diproses. Video akan muncul setelah berhasil diproses.</p>
                        </div>
                    </div>
                </SellerFormGroup>
        ...
        <SellerCard title="Informasi Penjualan">
            <div class="mt-4">
                <SellerFormGroup label="Variasi">
                    <div class="space-y-4">
                        <FeatureSellerProductVariant v-for="(variant, index) in variations" v-model="variant.name" v-model:value="variant.value" :key="`variations-${index}`" @delete="handleDeleteVariants(index)" />

                        <div class="p-4 bg-gray-100 rounded-sm">
                            <UButton color="white" class="text-primary" size="xs" icon="i-heroicons:plus" @click="handleAddVariation">
