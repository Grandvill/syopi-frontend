        ...
        <UCard>
            <div class="space-y-6">
                <div class="flex gap-4 items-center">
                    <UInput leading-icon="i-heroicons:magnifying-glass" class="flex-1" placeholder="Cari Nama Produk, SKU Induk, Kode Variasi, ID Produk" />
                    <USelectMenu :options="[]" placeholder="Cari berdasarkan kategori" class="flex-1" />
                    <UButton label="Terapkan" variant="outline" size="xs" />
                    <UButton label="Atur Ulang" color="white" size="xs" />
                </div>
                <p class="text-lg font-medium">0 Produk</p>
                <BaseDataTable :columns="columns" :rows="products" empty-text="Produk Tidak Ditemukan">
                <template #name-data="{ row }">
                    <div class="flex gap-2">
                        <img :src="row.images?.[0]" class="w-14 h-14 object-cover" />
                        <div class="flex flex-col items-start gap-0.5">
                            <span class="text-sm font-medium">{{ row.name }}</span>
                            <span class="text-sm text-black/65">ID Produk: {{ row.uuid }}</span>
                        </div>
                    </div>
                </template>
