<template>
  <form class="space-y-5 py-5">
    <SellerCard title="Rincian Dasar">
      <div class="space-y-4 mt-4">
        <SellerFormGroup label="Nama Voucher" help="Nama Voucher tidak ditampilkan ke Pembeli">
          <UInput v-model="form.name" size="lg" maxlength="100">
            <template #trailing>
              <span class="text-sm text-black/55">{{ `${form.name.length}/100` }}</span>
            </template>
          </UInput>
        </SellerFormGroup>
    ...
    <SellerCard title="Pengaturan Bonus">
      <div class="space-y-4 mt-4">
        ...
        <SellerFormGroup v-if="form.discount_cashback_type !== 'fixed' && form.voucher_type === 'discount'" label="Diskon/Cashback Maksimal">
