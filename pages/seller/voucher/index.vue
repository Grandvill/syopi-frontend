        ...
        <div class="mt-8 space-y-4 text-sm">
            ...
            <BaseDataTable :rows="items" :columns="columns">
                <template #name-data="{row}">
                    <div class="flex gap-2">
                        ...
                        <div class="flex flex-col items-start gap-0.5">
                            <UBadge label="Telah Berakhir" color="gray" size="xs" variant="soft"
                            :ui="{
                                variant: {
                                    soft: 'bg-gray-100'
                                }
                ...
                <template #discount-data="{row}">
                    <p>
                        {{ row.discount_cahsback_type === 'fixed' ? 'Rp' : "" }}{{ formatNumber(row.discount_cashback_value) }}{{ row.discount_cashback_type === 'percentage' ? '%' : "" }}
                    </p>
                    <div v-if="row.voucher_type === 'cashback'" class="flex gap-2 items-center text-sm text-black/55">
