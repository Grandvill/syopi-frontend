<template>
  <SellerCard title="Daftar Voucher">
    <template #header-append>
      <UButton icon="i-heroicons:plus" to="/seller/voucher/add">Tambah Voucher</UButton>
    </template>
    <BaseTabs
      :items="status"
      :ui="{
      ...
      <BaseDataTable :rows="items" :columns="columns">
        <template #name-data="{ row }">
          <div class="flex gap-2">
            <img class="w-14 h-14 object-cover" :src="row.voucher_type === 'discount' ? image[row.discount_cashback_type] : image['coin']" />
        ...
        <template #discount-data="{ row }">
          <p>{{ row.discount_cahsback_type === 'fixed' ? 'Rp' : '' }}{{ formatNumber(row.discount_cashback_value) }}{{ row.discount_cashback_type === 'percentage' ? '%' : '' }}</p>
          <div v-if="row.voucher_type === 'cashback'" class="flex gap-2 items-center text-sm text-black/55"><IconCoin class="w-4 h-4" /> Cashback Koin</div>
        </template>
